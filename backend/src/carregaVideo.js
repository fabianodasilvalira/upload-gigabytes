class HandlerUploadVideo {
  enviarVideoVimeo(saveFileTo) {
    uploadVideoVimeo(saveFileTo);
  }
}

module.exports = HandlerUploadVideo;

function uploadVideoVimeo(saveFileTo) {
  ...
  // let file_name = "downloads/pantera.mkvs";
  let file_name = saveFileTo;
  client.upload(
    file_name,
    {
  ...
  function statusTranscodificacao(uri) {
    ...
    client.request(
      ...
      function (error, body, statusCode, headers) {
        ...
        deletarVideoVimeo(saveFileTo);
      }
    );
  }
}

function deletarVideoVimeo(saveFileTo) {
  let fs = require("fs");

  fs.unlink(saveFileTo, function (err) {
    if (err) throw err;
